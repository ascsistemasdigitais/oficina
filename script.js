...
async function loadAllData() {
  try {
    // Carregar Usuários (SEM SENHAS)
    const userSnap = await getDocs(collection(db, "usuarios"));
    appData.usuarios = userSnap.docs.map(d => {
      const data = d.data();
      delete data.senha; // 🔒 Garante que nenhuma senha vazada do banco vá para a memória
      return { id: d.id, ...data };
    });

    // Se não houver usuários, criar registro do admin no Firestore (A senha deve ser criada no Firebase Auth!)
    if (appData.usuarios.length === 0) {
      ...
      console.log("⚠️ Registro admin criado no Firestore. Crie a conta no Firebase Authentication > Users com este email e uma senha forte!");
    }
...
// ==========================================
// 🔒 5. ALTERAR SENHA (SEGURO VIA FIREBASE AUTH)
// ==========================================
...
async function alterarSenha(e) {
  ...
  try {
    ...
    // 🔒 Reautenticação obrigatória pelo Firebase
    const credential = EmailAuthProvider.credential(user.email, senhaAtual);
    await reauthenticateWithCredential(user, credential);
    
    // 🔒 Atualiza a senha APENAS no Firebase Auth (NUNCA no Firestore)
...
// ==========================================
// 7. CRUD CLIENTES (COM XSS PROTECTION)
// ==========================================
...
// ==========================================
// 8. CRUD VEÍCULOS (COM XSS PROTECTION)
// ==========================================
...
function loadClientesSelect(selectId) {
  ...
  appData.clientes.forEach(c => {
    select.innerHTML += `<option value="${escapeHTML(c.id)}">${escapeHTML(c.nome)}</option>`;
  });
}
...
// ==========================================
// 9. ORDENS DE SERVIÇO (COM XSS PROTECTION)
// ==========================================
...
function viewOS(id) {
  ...
  document.getElementById('osPrintContent').innerHTML = `
    ...
    <div class="os-print-info">
      ...
      <div><label>CPF</label><span>${cliente ? escapeHTML(cliente.cpf) : '-'}</span></div>
...
function loadUsuarios() {
  ...
  appData.usuarios.forEach(u => {
    const tr = document.createElement('tr');
    // 🔒 Só permite excluir se for admin E não for o próprio usuário logado
...
async function saveUsuario(e) {
  ...
  try {
    let uid = null;
    // 🔒 Cria no Firebase Auth APENAS se for um novo usuário e tiver senha
    if (!id && senha) {
      try {
    ...
    // 🔒 Salva no Firestore SEM o campo senha
...
function editUsuario(id) {
  ...
  document.getElementById('usuarioSenha').value = ''; // 🔒 Nunca preenche a senha
  document.getElementById('usuarioSenha').placeholder = 'Deixe em branco para manter a atual';
  document.getElementById('usuarioSenha').disabled = true; // 🔒 Impede alteração por aqui
...
function fazerBackup() {
  // 🔒 Garante que nenhum dado sensível seja exportado
