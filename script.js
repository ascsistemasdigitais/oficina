...
async function loadAllData() {
  try {
    // Carregar Usuários (SEM SENHAS - apenas dados básicos)
    const userSnap = await getDocs(collection(db, "usuarios"));
    appData.usuarios = userSnap.docs.map(d => {
      const data = d.data();
      // 🔒 SEGURANÇA: Remove qualquer campo de senha que exista no Firestore
      delete data.senha;
      return { id: d.id, ...data };
    });
    ...
    if (appData.usuarios.length === 0) {
      ...
      console.log("⚠️ Usuário admin padrão criado no Firestore.");
      console.log("⚠️ IMPORTANTE: Crie esta conta no Firebase Authentication com uma senha forte!");
    }
...
// ==========================================
// 5. LOGIN COM FIREBASE AUTH (SEGURO)
// ==========================================
function setupLogin() {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      // 🔒 SEGURANÇA: Busca usuário pelo email (Firebase Auth já validou a senha)
      const found = appData.usuarios.find(u => u.login === user.email && u.status === 'ativo');

      if (found) {
        currentUser = found;
        currentUser.uid = user.uid; // Armazena o UID do Firebase Auth
  ...
  // Formulário de login - USA APENAS EMAIL E SENHA (Firebase Auth)
  document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    try {
      ...
      // 🔒 Login direto com Firebase Auth (senha validada de forma segura)
...
// ==========================================
// 🔒 ALTERAR SENHA (USANDO FIREBASE AUTH)
// ==========================================
...
async function alterarSenha(e) {
  ...
  try {
    ...
    // 🔒 Reautentica o usuário com a senha atual (obrigatório pelo Firebase)
    const credential = EmailAuthProvider.credential(user.email, senhaAtual);
    await reauthenticateWithCredential(user, credential);

    // 🔒 Atualiza a senha no Firebase Auth (único local seguro)
  ...
  appData.clientes
    ...
    .forEach(c => {
      ...
      tr.innerHTML = `
        ...
        <td>${c.email || '-'}</td>
        <td>
          <button class="btn-icon edit" onclick="editCliente('${c.id}')"><i class="fas fa-edit"></i></button>
          ${currentUser.tipo === 'admin' ? `<button class="btn-icon delete" onclick="deleteCliente('${c.id}')"><i class="fas fa-trash"></i></button>` : ''}
        </td>
      `;
...
function loadVeiculos() {
  ...
  appData.veiculos.filter(v => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn-icon edit" onclick="editVeiculo('${v.id}')"><i class="fas fa-edit"></i></button>
        ${currentUser.tipo === 'admin' ? `<button class="btn-icon delete" onclick="deleteVeiculo('${v.id}')"><i class="fas fa-trash"></i></button>` : ''}
      </td>
    `;
    tbody.appendChild(tr);
  });
...
function loadOrdens() {
  ...
  appData.ordens.forEach(o => {
    ...
    tr.innerHTML = `
      <td>${o.numero}</td>
      <td>${cliente ? cliente.nome : '-'}</td>
      <td>${veiculo ? `${veiculo.placa} - ${veiculo.marca}` : '-'}</td>
      <td>${formatDate(o.dataEntrada)}</td>
      <td>${formatCurrency(o.valorTotal)}</td>
      <td><span class="status-badge status-${o.status}">${getStatusLabel(o.status)}</span></td>
      <td>
        <button class="btn-icon view" onclick="viewOS('${o.id}')"><i class="fas fa-eye"></i></button>
        <button class="btn-icon edit" onclick="editOS('${o.id}')"><i class="fas fa-edit"></i></button>
        ${currentUser.tipo === 'admin' ? `<button class="btn-icon delete" onclick="deleteOS('${o.id}')"><i class="fas fa-trash"></i></button>` : ''}
      </td>
    `;
    tbody.appendChild(tr);
  });
...
function viewOS(id) {
  ...
  document.getElementById('osPrintContent').innerHTML = `
    <div class="os-print-header">
      <h2>${appData.dadosOficina.nome}</h2>
      <p>${appData.dadosOficina.endereco}</p>
...
function addServicoRow(data = null) {
  ...
  const tr = document.createElement('tr');
  tr.innerHTML = `
    <td><input type="text" class="serv-tipo" placeholder="Tipo de serviço" value="${data ? data.tipo : ''}"></td>
...
// ==========================================
// 🔒 10. USUÁRIOS (SEGURO - SEM SENHA NO FIRESTORE)
// ==========================================
function loadUsuarios() {
  ...
  appData.usuarios.forEach(u => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn-icon edit" onclick="editUsuario('${u.id}')"><i class="fas fa-edit"></i></button>
        ${u.login !== 'ratao.288@gmail.com' ? `<button class="btn-icon delete" onclick="deleteUsuario('${u.id}')"><i class="fas fa-trash"></i></button>` : ''}
      </td>
    `;
    tbody.appendChild(tr);
  });
...
async function saveUsuario(e) {
  ...
  try {
    // 🔒 CRIA O USUÁRIO NO FIREBASE AUTH (único local seguro para senhas)
    let uid = null;
    if (!id && senha) {
      try {
        ...
        if (authError.code === 'auth/email-already-in-use') {
          alert('⚠️ Este email já está em uso no Firebase Authentication. Verifique se o usuário já existe.');
          return;
...
function editUsuario(id) {
  ...
  document.getElementById('usuarioSenha').value = ''; // 🔒 Não preenche a senha
  document.getElementById('usuarioSenha').placeholder = 'Deixe em branco para manter a atual';
  document.getElementById('usuarioSenha').disabled = true; // 🔒 Desabilita edição
...
async function deleteUsuario(id) {
  if (confirm('Deseja realmente excluir este usuário?\n\n⚠️ ATENÇÃO: Isso NÃO remove a conta do Firebase Authentication. Você precisa fazer isso manualmente no Console do Firebase.')) {
    try {
      await deleteDocument('usuarios', id);
      appData.usuarios = appData.usuarios.filter(u => u.id !== id);
      loadUsuarios();
      alert('✅ Usuário removido do sistema.\n\n⚠️ Lembre-se de também remover a conta no Firebase Authentication > Users.');
    } catch (error) {
